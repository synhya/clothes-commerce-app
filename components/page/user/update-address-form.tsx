...
import { updateAddress, updateProfile } from '@/lib/actions/actions';
...
const UpdateAddressForm = ({ addressList }: {
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-6'>
        {fields.map((field, index) => (
          <div key={field.id} className='relative border rounded-md shadow-lg shadow-border px-4 pt-2 pb-4'>
            <div className='absolute top-3 right-3 flex gap-2 items-center'>
              ...
              {index !== fields.length - 1 && (
                ...
                />)}
              <Button
                type='button' disabled={form.formState.isSubmitting} size='icon' variant='destructive'
        ...
        <Button
          type='button' disabled={form.formState.isSubmitting} className='mr-2'
