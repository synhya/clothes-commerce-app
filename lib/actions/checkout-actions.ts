...
export type CheckoutActionProps = InvoiceFormValues & { lineItems: string; total_price: string };
...
export async function checkoutAction(formData: CheckoutActionProps) {
  ...
  const { error: lineItemError } = await supabase.from('invoice_products').insert(
    lineItems.map((item) => ({
      ...item,
